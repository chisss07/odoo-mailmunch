...
async def authenticate_odoo(url: str, db: str, login: str, api_key: str) -> dict:
    """Authenticate against Odoo via XML-RPC using API key. Returns uid."""
    _validate_odoo_url(url)
    url = url.rstrip("/")

    xml_args = (db, login, api_key, {})
